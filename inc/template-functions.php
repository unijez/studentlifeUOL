...
function default_image($thumbnail) {
	if ( has_post_thumbnail() ) {
        ...
        ?><img src="<?php display_thumbnail(); ?>" alt="<?php the_title(); ?>" /><?php
      } else {
 ...
 */
 ...
 function display_thumbnail() {
   preg_match('/src="(.+?)"/', get_field('youtube_video'), $matches); //Pull the URL of the video out of the iframe generated by ACF
   preg_match("/embed\/(.+)\?/", $matches[1], $vid_id); //Pull the ID of the video from the URL

   $display_url = "https://img.youtube.com/vi/".$vid_id[1]."/maxresdefault.jpg"; //Use the standard youtube link to get the max res thumbnail
